...
public class ApplyNetheriteInsetProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity, ItemStack itemstack) {
		...
		if (blockstate.is(BlockTags.create(ResourceLocation.parse("forge:chiseled"))) && (blockstate.getBlock().getStateDefinition().getProperty("blockstate") instanceof IntegerProperty _getip8 ? blockstate.getValue(_getip8) : -1) == 0) {
			if ((entity instanceof Player _plr ? _plr.getAbilities().instabuild : false) || (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(new ItemStack(Items.NETHERITE_SCRAP)) : false)) {
				...
				if (entity instanceof ServerPlayer _player) {
					AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("charming_chisels:pricey_flair"));
					if (_adv != null) {
